...
% Ensure legacy NeurIPS internal macro exists (fixes: Undefined control sequence \@noticestring -> \@trackname)
\makeatletter
\@ifundefined{@trackname}{\def\@trackname{}}{}
\makeatother

% Compile: run `pdflatex neurips_2025.tex` then `bibtex neurips_2025` then
% `pdflatex neurips_2025.tex` twice to resolve citations/refs.
...
\author{%
  ...
  \url{https://github.com/wangzh12023/NLPrompt}
}
...
% \section{Introduction}
% The advent of Vision-Language Models (VLMs) such as CLIP has bridged the gap between visual and textual data \citep{pan2024nlprompt}. Prompt learning has emerged as a parameter-efficient fine-tuning method for these models \citep{pan2024nlprompt}. However, real-world datasets are inherently noisy, and standard Cross-Entropy (CE) loss is known to overfit to incorrect labels, leading to performance degradation \citep{pan2024nlprompt}.
% The paper \textit{NLPrompt: Taming Noisy Labels in Vision-Language Models} \citep{pan2024nlprompt} addresses this by employing Optimal Transport (OT) to align image features with text prototypes, thereby identifying and "purifying" noisy labels. The original method partitions data into "clean" and "noisy" subsets, applying CE loss to the former and Mean Absolute Error (MAE) loss to the latter \citep{pan2024nlprompt, zhang2018gce}.
% In this project, we aim to:
% \begin{enumerate}
%     \item \textbf{Replicate} the NLPrompt framework and verify its performance on standard benchmarks.
...
% On the Food101N dataset, our implementation achieved an accuracy of \textbf{72.9\%}, outperforming the CoOp baseline \citep{pan2024nlprompt}.
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%TODO%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{Conclusion}
In this project, we successfully reproduced \textit{NLPrompt}, validating its effectiveness against noisy labels in VLMs. From a convex optimization perspective, we rigorously analyzed the OT formulation via Bregman Projections and addressed the challenge of class imbalance by mathematically deriving the correct marginal constraints that define the transport polytope. Finally, to overcome the information loss in "hard partitioning", we proposed \textbf{OT-Guided Adaptive Generalized Cross Entropy (OT-AdaGCE)}, a soft-weighting strategy that dynamically adjusts loss robustness based on optimal transport confidence.
...
% NOTE: Rerun LaTeX to update cross-references and outlines if you see warnings.
\end{ack}

\small
\clearpage
\bibliographystyle{plainnat} % ensure natbib-compatible style for BibTeX runs
...
% NOTE: If you see warnings about labels or outlines, rerun LaTeX to update cross-references and outlines.
